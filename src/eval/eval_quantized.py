...
# --- ADD: utility robusta per torch.load su PyTorch 2.6+ ---
def _torch_load_trusted(path, map_location="cpu"):
    """
    Carica un checkpoint/artefatto *fidato* disabilitando weights_only (PyTorch >=2.6).
    Fallback per versioni precedenti dove l'argomento non esiste.
    """
    try:
...
def main():
    ...
    ap.add_argument("--latents", required=True)        # z.pt con ordine val
    ap.add_argument("--codes", required=True)           # codes.npy stesso ordine di z.pt
    ap.add_argument("--codebook", required=True)        # codebook.pt (contiene z_medoid)
    ...
    codes = torch.from_numpy(__import__("numpy").load(args.codes)).long()
    assert len(codes)==len(z_all), "codes.npy e z.pt devono avere stessa lunghezza/ordine"

    # costruisci zq per-campione
    zq_all = z_medoid[codes]

    # batch inferenza per metriche
    bs = 256
