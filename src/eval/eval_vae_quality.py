...
def evaluate_latent_reconstructions(model: VAE, latents: torch.Tensor, 
                                   data_config: dict, device: torch.device,
                                   max_samples: int = 2048, batch_size: int = 512) -> torch.Tensor:
...
def main():
    ...
    parser.add_argument('--latents_dir', required=True, help='Directory containing mu.pt and z.pt')
    parser.add_argument('--vae_config', default='configs/vae_fashion.yaml', help='VAE config file')
    parser.add_argument('--data_config', default='configs/data_fashion.yaml', help='Data config file')
    parser.add_argument('--max_samples', type=int, default=2048, help='Max samples to evaluate')
