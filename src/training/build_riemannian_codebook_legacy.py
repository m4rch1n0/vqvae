...
def _load_vae_model(checkpoint_path: Path, dataset: str, device: torch.device) -> VAE:
    ...
    # Load VAE config from the consolidated train.yaml file
    train_config_path = f"configs/presets/{dataset}/train.yaml"
    import yaml
...
def build_and_save(config: Dict) -> Path:
    ...
    # Parse checkpoint path (try multiple locations for flexibility)
    checkpoint_path_cfg = (config.get("checkpoint_path") or 
                          config.get("vae", {}).get("ckpt_path") or
                          (config.get("model", {}).get("checkpoint_path") if isinstance(config.get("model"), dict) else None))
    ...
    # Get VAE config from config file (supports both 'vae_config' and 'model' keys)
    vae_config = config.get("vae_config") or config.get("model") or config.get("vae")
    if vae_config is None:
        raise ValueError("VAE configuration not found. Expected 'vae_config', 'model', or 'vae' key in config.")
    
    vae_model = _load_vae_model(checkpoint_path, vae_config, device)
...
def _load_vae_model(checkpoint_path: Path, vae_config: Dict, device: torch.device) -> VAE:
    """Load trained VAE model from checkpoint using provided config.""" 
    checkpoint = torch.load(checkpoint_path, map_location=device)
    ...
    # Use provided VAE config directly
    model = VAE(**vae_config)
    model.load_state_dict(state_dict)
    model.to(device)
    model.eval()
